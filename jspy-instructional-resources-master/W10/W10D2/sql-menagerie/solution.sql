...
-- \q
-- cat ./sql-menagerie/cities_and_airports.sql | psql -d travel -U bryce
-- PHASE 2, QUERY #1
SELECT city,
  state,
...
SELECT city,
  population_estimate_2018 / 1000000 as pop_estimate_2018_millions
FROM cities
WHERE state = 'Texas';
-- RESULTS:
--     city     | ?column?
...
-- -- Cities on left, airports on right
-- SELECT *
-- FROM cities
-- INNER JOIN airports
...
SELECT city,
  state,
  to_char(population_estimate_2018, '9,999,999')
FROM cities;
-- RESULTS:
--      city      |        state         |  to_char
-- ---------------+----------------------+------------
...
-- (25 rows)
-- BONUS #3
-- Without the .0 or some other form of data type casting, our division of two integers results in an integer
SELECT city,
  round(population_estimate_2018 / 1000000.0, 2)
FROM cities
WHERE state = 'Texas';
-- RESULTS:
--     city     | round
