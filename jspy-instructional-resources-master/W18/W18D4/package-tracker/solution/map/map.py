...
def find_shortest_path_dij(start, end):
    # Track list of citiest that we need to visit next
    next_cities = [start]
    ...
    while len(next_cities) and end not in visited:
        ...
        for city, next_distance in map_dij[current]:
...
def advance_delivery(location, destination):
    ...
    path = find_shortest_path_bfs(location, destination)
...
print("Dij:", find_shortest_path_dij("Seattle", "Washington D.C."))  # Dij: ['Seattle', 'San Francisco', 'Denver', 'Kansas City', 'Chicago', 'New York', 'Washington D.C.']
