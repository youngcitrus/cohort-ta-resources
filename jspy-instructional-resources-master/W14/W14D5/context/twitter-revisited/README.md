...
API, you'll want to set up an API base url. You can do so by adding [custom
environment variables] to your React application. The idea is to follow the same
pattern that you used in Express.

You'll begin by creating an `.env` file in the root of your frontend directory
...
The next step is to create a `config.js` module in your `src` directory:

```js
// ./twitter-front-end/src/config.js
export const apiBaseUrl = process.env.REACT_APP_API_BASE_URL;
```

Now you can import the `REACT_APP_API_BASE_URL` environment variable into any
module that references `http://localhost:8080`:

```js
import { apiBaseUrl } from './config';
```

You can use your `apiBaseUrl` whenever you need to make a request to help clean
up your code! For example:

```js
// With `REACT_APP_API_BASE_URL` environment variable
...
// Without `REACT_APP_API_BASE_URL` environment variable
...
use of your `apiBaseUrl` environment variable to make a request to
`${apiBaseUrl}/users` instead of `http://localhost:8080/users`.
